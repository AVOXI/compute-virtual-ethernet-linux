...
#define GVE_VERSION		 "1.4.1-1-df98a34-9e820ea-oot"
...
#else /* LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0) */
	qpls = kcalloc(max_queues, sizeof(*qpls), GFP_KERNEL);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0) */
	if (!qpls)
		return -ENOMEM;
...
int gve_set_buffer_size_config(struct gve_priv *priv, bool enable_hdr_split,
	...
	gve_get_curr_alloc_cfgs(priv, &qpls_alloc_cfg, &tx_alloc_cfg,
				&rx_alloc_cfg);
	...
	if (netif_running(priv->dev)) {
		err = gve_adjust_config(priv, &qpls_alloc_cfg, &tx_alloc_cfg,
					&rx_alloc_cfg);
	}
	return err;
}
